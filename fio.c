...
U wr(I d,S b,U n){R write(d,b,n);}
...
U fsz(S f){ST(b);I r=stat(f,&b);QQ(r,f)R b.st_size;}
...
K map(S f,U*n,I d){ST(b);d=d?rwd(d,0):open(f,O_RDONLY,0);fstat(d,&b);S s=mmap(0,*n=b.st_size,PROT_READ,MAP_PRIVATE,d,0);R close(d),(K)(16+s);}

I opr(K x){S f=cS(x);I r=acc(f,0);Z(!r,goto OP);r=mkfifo(f,0600);QQ(-1==r,"fifo")OP:ffo(f);R r=open(f,O_RDONLY,0),free(f),r;}
I*opw(K x,K y){dir(x);K z,r=tn(KI,yn);S hdr=calloc(1,16);N(yn,z=str(Yu);Ri=open(zG,O_RDWR|O_CREAT|O_TRUNC,0600);QQ(3>Ri,zG)wr(Ri,hdr,16);r0(z);)R free(hdr),r;}

K cork(I*fds,K r,U rct){I d;U y,x[2];N(rn,d=rwd(fds[i],0);y=Ru;xx=16+BSZ(yt,rct);xy=yU[-1];wr(d,xG,16),Ru=y=map(0,x,d);)R r;}
